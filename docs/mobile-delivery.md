...
credentials command opens EAS iOS signing setup. The register command then opens Expo's device
registration flow so the iPhone UDID can be added to the iOS provisioning profile. Apple requires
this for ad hoc/internal iOS install links.
