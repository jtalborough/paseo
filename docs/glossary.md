...
- **Project** — User-authored domain container with a stable `grp_` id, a Paseo-managed directory, and zero or more external Folders. UI: "Project" / "New Project". Code: `ProjectGroupPayload` (`packages/protocol/src/messages.ts`). Project content keys to `groupId`, never a Folder's legacy `projectId`.
- **Folder** — External git repository or ordinary directory referenced by a Project. A Folder is not owned, moved, or deleted with its Project. Code currently retains legacy `projectId` naming for Folder identity.
- **Workspace** — One concrete `cwd` on one daemon; belongs to exactly one Folder. UI: "Workspace". Code: `WorkspaceDescriptorPayload` (`packages/protocol/src/messages.ts`). Don't confuse with: Branch (one branch can back many workspaces via worktrees).
...
- **Project host entry** — Legacy aggregated Folder view for one `(projectKey, daemon)` pair. Internal. Code: `ProjectHostEntry` (`packages/app/src/utils/projects.ts:11`). This predates user-authored domain Projects.
