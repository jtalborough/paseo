...
import { recordTaskScheduleRun } from "./task/schedule-run-ledger.js";
...
export async function createPaseoDaemon(
  ...
  const contextPacketStore = new ProjectContextPacketStore(config.paseoHome);
  const scheduleService = new ScheduleService({
    ...
    agentStorage,
    providerSnapshotManager,
    onRunStarted: (schedule, run) =>
      recordTaskScheduleRun({ taskStore, contextPacketStore, schedule, run }),
    onRunFinished: (schedule, run) =>
      recordTaskScheduleRun({ taskStore, contextPacketStore, schedule, run }),
  });
