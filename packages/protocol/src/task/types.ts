 ...
 * `$PASEO_HOME/projects/<groupId>/tasks/<id>.md`. The frontmatter holds the
...
export const TaskFrontmatterSchema = z
  .object({
    ...
    id: z.string().min(1),
    /** The user-authored Project this task belongs to. */
    ...
    timerStartedAt: nullableString(),
    /** Accumulated whole seconds from completed stopwatch intervals. */
    trackedSeconds: z.number().int().nonnegative().default(0),
    /** Append-only time intervals used for daily/project timesheet aggregation. */
 ...
 * pickers. Stored at `projects/<groupId>/tasks/task-config.json`. Free-form: the
...
export const TaskViewFilterSchema = z.object({
  active: z.boolean().optional(),
  doDate: z.enum(["due", "missing", "any"]).optional(),
  actionStates: z.array(ActionStateSchema).optional(),
  priorities: z.array(TaskPrioritySchema).optional(),
  projectGroupIds: z.array(z.string().regex(/^grp_[A-Za-z0-9_-]+$/)).optional(),
  timer: z.enum(["running", "stopped", "any"]).optional(),
});
...
export const TaskViewsSchema = z.array(TaskViewDefinitionSchema);
export type TaskViewDefinition = z.infer<typeof TaskViewDefinitionSchema>;
